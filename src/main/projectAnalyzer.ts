...
const TOP_LEVEL_PRIORITY = ['apps', 'packages', 'services', 'server', 'api', 'backend', 'web', 'app', 'client', 'frontend', 'src', 'db', 'database', 'prisma']
const REPRESENTATIVE_FILES = ['package.json', 'README.md', 'index.ts', 'index.tsx', 'main.ts', 'main.tsx', 'server.ts', 'server.js', 'app.ts', 'app.tsx', 'index.html', 'schema.prisma', '__init__.py', 'db.py']
const ALLOWED_ICON_NAMES = new Set([
  'Monitor', 'Shield', 'Lock', 'Network', 'Globe', 'ArrowLeftRight', 'GitBranch', 'Webhook',
  'Settings2', 'Brain', 'Layers', 'Cpu', 'Clock', 'Mail', 'Bell', 'CreditCard', 'Search',
  'Activity', 'BarChart2', 'ToggleLeft', 'Database', 'Zap', 'Archive', 'Table', 'Boxes',
  'Share2', 'TrendingUp', 'Wrench',
])
...
const PACKAGE_ROLE_DEFINITIONS: RoleDefinition[] = [
  {
    ...
    contracts: 'Routes, MCP tools, request/response shapes, websocket APIs, and server lifecycle boundaries.',
    prompt: 'Continue from the existing server and API implementation. Inspect request flows, preserve contracts, and only plan or apply the next required delta.',
    reviewHints: 'Inspect exposed endpoints, websocket handlers, MCP tools, and server bootstrap paths before editing.',
    filePatterns: [/(^|\/)(api|server|web_server|mcp_server|gateway|routes?)\.py$/i],
  },
  {
    key: 'workflow',
    label: 'Worker',
    ...
    color: '#94a3b8',
    tag: 'WORKER',
    description: 'Owns automation, replay, background execution, or workflow orchestration for this package.',
    contracts: 'Execution flow, task orchestration, Playwright/runtime behavior, and transformation boundaries.',
    prompt: 'Continue from the existing workflow and automation implementation. Inspect the current execution path first, then make only the next required delta.',
    reviewHints: 'Inspect orchestration, replay flow, capture/summarize steps, and CLI entrypoints before editing.',
    filePatterns: [/(^|\/)(record|runner|playwright|capture|summarize|template|worker|jobs?)\.py$/i],
  },
  {
    key: 'storage',
    label: 'Database',
    ...
    contracts: 'Database schema, storage adapters, repositories, and persistence contracts.',
    prompt: 'Continue from the existing persistence layer. Inspect current schema and storage boundaries before making changes.',
    reviewHints: 'Inspect schema creation, query helpers, repositories, and data access patterns before editing.',
    filePatterns: [/(^|\/)(db|database|storage|store|schema|models?|repositories?)\.py$/i],
  },
]
...
function collectRepresentativeFiles(projectDir: string, relDir: string, limit = 4): string[] {
...
function inferCategory(pathFragment: string): ImportedNodeCategory {
  if (/(db|database|prisma|migrations?|schema|postgres|mysql|mongo|redis|cache)/i.test(pathFragment)) return 'storage'
  if (/(api|server|backend|service|worker|queue|auth|gateway)/i.test(pathFragment)) return 'services'
  if (/(web|client|frontend|ui|renderer|desktop|mobile|app)/i.test(pathFragment)) return 'infrastructure'
  return 'custom'
}

function inferVisuals(relPath: string, category: ImportedNodeCategory) {
  if (category === 'storage') return { iconName: 'Database', color: '#60a5fa', tag: 'DATA' }
  if (/auth/i.test(relPath)) return { iconName: 'Lock', color: '#34d399', tag: 'AUTH' }
  if (/(api|gateway)/i.test(relPath)) return { iconName: 'Shield', color: '#fb923c', tag: 'API' }
  if (/(web|client|frontend|ui|renderer|desktop)/i.test(relPath)) return { iconName: 'Monitor', color: '#f472b6', tag: 'UI' }
  if (/(worker|queue|jobs?)/i.test(relPath)) return { iconName: 'Cpu', color: '#a78bfa', tag: 'JOB' }
  if (category === 'services') return { iconName: 'Globe', color: '#34d399', tag: 'SVC' }
  return { iconName: 'Layers', color: '#facc15', tag: 'MOD' }
}

function inferDescription(relPath: string, category: ImportedNodeCategory) {
  if (category === 'storage') return `Owns the existing persistence layer under ${relPath}.`
  if (category === 'services') return `Owns the existing service implementation under ${relPath}.`
  if (category === 'infrastructure') return `Owns the existing application surface under ${relPath}.`
  return `Owns the existing project area under ${relPath}.`
}

function inferContracts(relPath: string, category: ImportedNodeCategory) {
  if (category === 'storage') return `Schema, migrations, repositories, and data access boundaries rooted in ${relPath}.`
  if (category === 'services') return `Endpoints, handlers, service interfaces, and integration contracts rooted in ${relPath}.`
  if (category === 'infrastructure') return `Routes, UI/state boundaries, and integration points owned under ${relPath}.`
...
function buildPackageRoleNodes(projectDir: string, packageRelPath: string): ImportedProjectNode[] {
  const packageName = packageRelPath.split('/').pop() || packageRelPath
  const files = readTopLevelFiles(projectDir, packageRelPath)
  const grouped = PACKAGE_ROLE_DEFINITIONS
    .map(role => ({
      role,
      files: files.filter(file => role.filePatterns.some(pattern => pattern.test(file))),
    }))
    .filter(group => group.files.length > 0)

  if (grouped.length <= 1) return []

  return grouped.map(group => makeRoleNode(packageName, packageRelPath, group.role, group.files, group.files))
}

function makeNode(projectDir: string, relPath: string): ImportedProjectNode {
  const baseName = relPath === '.' ? 'workspace' : relPath.split('/').pop() || relPath
  ...
  return {
    ...
    reviewHints: 'Inspect the current implementation before editing. Preserve existing structure and only make deltas inside owned paths unless the task explicitly requires a contract change.',
  }
}

function mergeNodeData(base: ImportedProjectNode, extra: ImportedProjectNode): ImportedProjectNode {
  return {
    ...base,
    ownedPaths: [...new Set([...base.ownedPaths, ...extra.ownedPaths])],
    expectedFiles: [...new Set([...base.expectedFiles, ...extra.expectedFiles])].slice(0, 6),
    contracts: [base.contracts, extra.contracts].filter(Boolean).join(' '),
    reviewHints: [base.reviewHints, extra.reviewHints].filter(Boolean).join(' '),
  }
}

function consolidateStorageNodes(nodes: ImportedProjectNode[]) {
  const roleStorageNodes = nodes.filter(node => node.tag === 'DB')
  const genericStorageNodes = nodes.filter(node =>
    node.category === 'storage'
    && node.tag !== 'DB'
    && node.ownedPaths.some(path => /^(db|database|prisma)$/i.test(path))
  )

  if (roleStorageNodes.length !== 1 || genericStorageNodes.length === 0) return nodes
...
function detectCandidateNodes(projectDir: string): ImportedProjectNode[] {
  const topLevelEntries = safeReadDir(projectDir).filter(entry => entry.isDirectory() && !isIgnoredDirectory(entry.name))
  ...
  for (const groupName of ['apps', 'packages']) {
    ...
    for (const entry of safeReadDir(join(projectDir, groupName))) {
      ...
      if (hasCodeFiles(join(projectDir, relPath))) genericCandidates.push(relPath)
    }
  }
  ...
  if (genericCandidates.length === 0) {
    for (const entry of topLevelEntries) {
      if (isTestDirectory(entry.name)) continue
      if (hasCodeFiles(join(projectDir, entry.name))) genericCandidates.push(entry.name)
    }
  }
  ...
  if (nodes.length > 0) {
    const testsEntry = topLevelEntries.find(entry => isTestDirectory(entry.name))
    if (testsEntry && hasCodeFiles(join(projectDir, testsEntry.name))) {
      nodes.push({
        id: sanitizeId(testsEntry.name),
        label: 'Tests',
        description: 'Owns test coverage and verification for the existing implementation.',
        category: 'services',
        iconName: 'Activity',
        color: '#22c55e',
        tag: 'TEST',
        ...
        contracts: 'Verification coverage, regression checks, and executable examples for the existing system.',
        reviewHints: 'Treat tests as downstream verification. Align them to the imported implementation nodes instead of defining the architecture from tests outward.',
      })
    }
  }
...
function buildEdges(nodes: ImportedProjectNode[]): ImportedProjectEdge[] {
  const frontends = nodes.filter(node => node.tag === 'UI' || /web|client|frontend|renderer|desktop/i.test(node.id))
  const services = nodes.filter(node => (
    node.category === 'services' || node.tag === 'API' || node.tag === 'AUTH' || node.tag === 'WORKER'
  ) && node.tag !== 'TEST')
  ...
  const preferredService = services.find(node => node.tag === 'API') || services[0]
  const preferredStorage = storage[0]
  const primaryExecutionNode = services.find(node => node.tag === 'WORKER') || preferredService || frontends[0]
  ...
  for (const testNode of tests) {
    for (const candidate of nodes.filter(node => node.id !== testNode.id && node.tag !== 'TEST')) {
...
function detectLanguages(paths: string[]) {
  ...
  for (const relPath of paths) {
    ...
    else if (/\.(sql|prisma|db|sqlite|sqlite3)$/.test(lower)) languages.add('storage')
  }
  return languages
}
...
function buildBoundaryReasons(node: ImportedProjectNode) {
  ...
  if (node.tag === 'DB' || node.category === 'storage') reasons.push('storage, schema, or database markers')
  if (node.expectedFiles.some(file => file.endsWith('package.json'))) reasons.push('package manifest')
  if (reasons.length === 0) reasons.push('directory and file naming signals')
  return reasons
}

function collectRepresentativeSamples(projectDir: string, nodes: ImportedProjectNode[]): RepresentativeSample[] {
...
function collectDirectoryTree(projectDir: string, maxDepth = 4, maxEntries = 220) {
  ...
  const walk = (absDir: string, depth: number, prefix: string) => {
    ...
    const entries = safeReadDir(absDir)
      .filter(entry => entry.isDirectory() ? !isIgnoredDirectory(entry.name) : isImplementationFile(entry.name))
      .sort((a, b) => {
...
function buildStructureSummary(projectDir: string, nodes: ImportedProjectNode[]): ProjectStructureSummary {
  const topLevelEntries = safeReadDir(projectDir).filter(entry => !isIgnoredDirectory(entry.name))
  const topLevelFiles = topLevelEntries
    .filter(entry => entry.isFile() && isImplementationFile(entry.name))
    .map(entry => entry.name)
    .slice(0, 12)
  const candidateBoundaries: StructureCandidateBoundary[] = nodes.map(node => ({
  ...
  if (nodes.length >= 2 && nodes.length <= 8) score += 2
    ...
    score -= 1
    notes.push('The repository likely contains many boundaries or noisy folders.')
  }

  if (rootImplementationFileCount >= 8) {
    score -= 2
    notes.push('The repository has many implementation files at the root, so folder boundaries may be weak.')
  }

  if (nodes.some(node => node.tag === 'TEST')) {
    notes.push('Tests were treated as downstream verification instead of primary architecture.')
  }

  const storageCandidates = nodes.filter(node => node.category === 'storage' || node.tag === 'DB')
  if (storageCandidates.length > 1) {
    score -= 1
    notes.push('Multiple storage markers were detected and may need consolidation.')
  }
  ...
  if (nodes.some(node => ['API', 'WORKER', 'UI'].includes(node.tag))) score += 1
  if (representativeSamples.length >= 3) score += 1

  const confidence: ProjectBootstrapConfidence = score >= 3 ? 'high' : score >= 1 ? 'medium' : 'low'
...
function shouldUseAgentImport(summary: ProjectStructureSummary, nodes: ImportedProjectNode[]) {
...
function buildResultSummary(nodes: ImportedProjectNode[], source: ProjectBootstrapResult['source']) {
  ...
  if (source === 'agent') return `${base} Architecture was synthesized from the repo structure with the import agent.`
  if (source === 'fallback') return `${base} Agent synthesis fell back to deterministic structure analysis.`
...
function isImportedCategory(value: unknown): value is ImportedNodeCategory {
  return value === 'infrastructure' || value === 'services' || value === 'storage' || value === 'custom'
}
...
function findFallbackNode(raw: Record<string, unknown>, fallbackNodes: ImportedProjectNode[]) {
  const rawId = sanitizeId(String(raw.id ?? ''))
  const rawLabel = String(raw.label ?? '').trim().toLowerCase()
  const rawPaths = normalizeRelativePaths(Array.isArray(raw.ownedPaths) ? raw.ownedPaths.filter((value): value is string => typeof value === 'string') : [])
...
function normalizeImportedNode(
  ...
  const expectedFiles = normalizeRelativePaths(Array.isArray(raw.expectedFiles) ? raw.expectedFiles.filter((value): value is string => typeof value === 'string') : fallback?.expectedFiles)
  const ownedPaths = normalizeRelativePaths(Array.isArray(raw.ownedPaths) ? raw.ownedPaths.filter((value): value is string => typeof value === 'string') : fallback?.ownedPaths)
  const effectiveOwnedPaths = ownedPaths.length > 0 ? ownedPaths : deriveOwnedPaths(expectedFiles)
  const subject = effectiveOwnedPaths[0] || expectedFiles[0] || String(raw.label ?? fallback?.label ?? `node-${index}`)
  const category = isImportedCategory(raw.category) ? raw.category : (fallback?.category ?? inferCategory(subject))
  const visuals = inferVisuals(subject, category)
  const id = sanitizeId(String(raw.id ?? fallback?.id ?? raw.label ?? `imported-node-${index}`))
  const label = truncate(String(raw.label ?? fallback?.label ?? titleCase(subject.replace(/\/.*/, ''))), 48)
  ...
  return {
    id,
    label,
    description: truncate(String(raw.description ?? fallback?.description ?? inferDescription(subject, category)), 180),
    category,
    iconName: typeof raw.iconName === 'string' && ALLOWED_ICON_NAMES.has(raw.iconName) ? raw.iconName : (fallback?.iconName ?? visuals.iconName),
    color: typeof raw.color === 'string' && isValidHexColor(raw.color) ? raw.color : (fallback?.color ?? visuals.color),
    tag: normalizeTag(String(raw.tag ?? fallback?.tag ?? visuals.tag), fallback?.tag ?? visuals.tag),
    prompt: truncate(String(raw.prompt ?? fallback?.prompt ?? inferPrompt(subject)), 420),
    ownedPaths: effectiveOwnedPaths.length > 0 ? effectiveOwnedPaths.slice(0, 6) : (fallback?.ownedPaths ?? ['.']),
    expectedFiles: expectedFiles.length > 0 ? expectedFiles.slice(0, 6) : (fallback?.expectedFiles ?? []),
    contracts: truncate(String(raw.contracts ?? fallback?.contracts ?? inferContracts(subject, category)), 320),
    reviewHints: truncate(String(raw.reviewHints ?? fallback?.reviewHints ?? 'Inspect the current implementation before editing. Preserve existing structure and only make deltas inside owned paths unless the task explicitly requires a contract change.'), 320),
  }
}
...
function normalizeImportedGraph(
  ...
  const normalizedNodes = dedupeNodes(
    rawNodes
      .map((item, index) => typeof item === 'object' && item !== null
        ? normalizeImportedNode(item as Record<string, unknown>, index, fallbackNodes)
        : null)
  ...
  for (const item of rawEdges) {
    ...
    const source = sanitizeId(String((item as Record<string, unknown>).source ?? ''))
    const target = sanitizeId(String((item as Record<string, unknown>).target ?? ''))
  ...
  return {
    ...
    edges: normalizedEdges.length > 0 ? normalizedEdges : buildEdges(normalizedNodes),
  }
}
...
function parseImportBlock(rawOutput: string) {
  ...
  const end = output.indexOf(IMPORT_BLOCK_END, start + IMPORT_BLOCK_START.length)
...
function buildImportPrompt(structure: ProjectStructureSummary, deterministicNodes: ImportedProjectNode[]) {
...
async function synthesizeArchitectureFromAgent(
...
): Promise<{ graph: { nodes: ImportedProjectNode[]; edges: ImportedProjectEdge[] } | null; error?: string }> {
  ...
  if (!payload) {
    return { graph: null, error: result.error ?? 'Import agent did not return a valid architecture block.' }
  }

  const graph = normalizeImportedGraph(payload, deterministicNodes)
  if (!graph) {
    return { graph: null, error: 'Import agent returned an empty or invalid graph.' }
  }
...
export async function bootstrapProjectCanvas(
  ...
  const synthesis = await synthesizeArchitectureFromAgent(projectDir, runtime, structure, deterministicNodes)
  if (synthesis.graph) {
    return {
  ...
  return {
    nodes: deterministicNodes,
    edges: deterministicEdges,
    summary: `${buildResultSummary(deterministicNodes, 'fallback')} ${synthesis.error ?? ''}`.trim(),
    source: 'fallback',
    confidence: structure.confidence,
    notes: structure.notes,
  }
