...
// shell commands themselves. Heredoc-based writes are fragile under any
// command-rewriter (rtk, ssh -c, screen send-keys, etc.) because the agent has
// to triple-escape single quotes around the heredoc terminator. Argv-based
// writes survive any such wrapper because every shell layer re-quotes argv
// correctly.
//
...
if command -v python3 >/dev/null 2>&1; then
  python3 - "$TS" "$ARCHITECT_PARTICIPANT_ID" "$KIND" "$TASK_ID" "$CONTENT" "$STRUCTURED" "$ARCHITECT_ACTIVITY_LOG" <<'PYEOF'
import json, sys
ts, pid, kind, tid, content, structured, log = sys.argv[1:]
