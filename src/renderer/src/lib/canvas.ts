...
export function normalizeNodeData(raw: unknown, settings: ProjectSettings): ArchitectNodeData {
  ...
  return {
    ...
    additionalChanges: typeof rawData.additionalChanges === 'string' ? rawData.additionalChanges : defaultConfig.additionalChanges,
    ownedPaths: Array.isArray(rawData.ownedPaths) ? rawData.ownedPaths.filter((value): value is string => typeof value === 'string') : defaultConfig.ownedPaths,
    expectedFiles: Array.isArray(rawData.expectedFiles) ? rawData.expectedFiles.filter((value): value is string => typeof value === 'string') : defaultConfig.expectedFiles,
    contracts: typeof rawData.contracts === 'string' ? rawData.contracts : defaultConfig.contracts,
    reviewHints: typeof rawData.reviewHints === 'string' ? rawData.reviewHints : defaultConfig.reviewHints,
