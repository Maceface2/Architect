...
function parseCanvasUpdateBlock(jsonStr: string): CanvasUpdate | null {
  const candidates = [jsonStr.trim(), repairWrappedJson(jsonStr.trim())];
...
export default function AssistantPanel({
  ...
  const parseForUpdates = useCallback(
    (raw: string) => {
      ...
      while ((startIdx = parseBufferRef.current.indexOf(START)) !== -1) {
        const endIdx = parseBufferRef.current.indexOf(
          END,
          startIdx + START.length,
        );
        if (endIdx === -1) break;

        const jsonStr = parseBufferRef.current
          .slice(startIdx + START.length, endIdx)
          .trim();
        parseBufferRef.current = parseBufferRef.current.slice(
          endIdx + END.length,
        );
